...
6. **Promote candidates** — check if any critical/important lessons should become global guidelines:

For each lesson with `severity: critical` or `category: pattern-discovered`:
- Ask the user: "Should this lesson become a global guideline? It would apply to all future projects."
- If yes, promote it:
```bash
python -m core.lessons promote {lesson_id} --scope {appropriate_scope} --weight {must|should}
```

Severity maps to suggested weight: `critical → must`, `important → should`, `minor → may`.

Show the user the promoted guidelines:
```bash
python -m core.guidelines read _global
```
...
- After recording, always check for promotion candidates (step 6)
