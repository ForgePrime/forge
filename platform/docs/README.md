...
| [`PLAN_MEMORY_CONTEXT.md`](PLAN_MEMORY_CONTEXT.md) | DRAFT | no | Phase B (7 stages): CausalEdge DAG, CausalGraph, ContextProjector + **B.5 TimelyDeliveryGate (ECITP C3)** + **B.6 SemanticRelationTypes (ECITP C6)** + **B.7 SourceConflictDetector (FC §8)**. Closes CCEGAP **1**+**3**; ECITP **C3**+**C6**; FC **§8**. |
...
| [`PLAN_CONTRACT_DISCIPLINE.md`](PLAN_CONTRACT_DISCIPLINE.md) | DRAFT | no | Phases E+F (20 stages): ContractSchema, Invariants, Autonomy, assumption enforcement, BLOCKED state + **E.7 EpistemicProgressGate (ECITP C8 + §2.7)** + **E.8 ScopeBoundaryDeclaration (FC §15)** + **F.10 StructuredTransferGate (ECITP C11)** + **F.11 CandidateSolutionEvaluation (FC §16-§19)** + **F.12 TechnicalDebtTracking (FC §37)**. Closes CCEGAP **2**+**4**+**7**; ECITP **C8**+**C11**+**§2.4**+**§2.7**; FC **§15**+**§16-§19**+**§37**. |
| [`PLAN_GOVERNANCE.md`](PLAN_GOVERNANCE.md) | DRAFT | no | Phase G (10 stages): CGAID capstone + **G.9 ProofTrailCompleteness (ECITP C7, C12)** + **G.10 BaselinePostVerification (FC §25+§26)** + ECITP C6/C11 REJECT-promotion. Terminal gate verifies 7 CCEGAP + 6 ECITP + 3 ECITP continuity + 5 FC critical-gap conditions = **21 mechanical checks** at per-execution level. System-level soundness (soak, multi-agent) out of scope (disclosed). |
...
| [`THEOREM_VERIFICATION_AIOS_AISDLC.md`](THEOREM_VERIFICATION_AIOS_AISDLC.md) | DRAFT | no | Merged verification of Forge against AIOS (24 axioms) + AI-SDLC (25 conditions) theorems. Aggregate: AIOS 12/24 ✅ · AI-SDLC 19/25 ✅. Zero fundamental gaps; 3 shared partials (CritPath, ErrorProp, Actor/Process) identified for Tier-1 closure via proposed ADR-023/024/025. 4 AIOS axioms (A11/A12/A19 + A2-A4) JustifiedNotApplicable or DEFER with explicit scope reasoning. Integrates with §26 Forge design characterization. |
