...
1. **[CLOSED 2026-04-24] §11 graph is narrative, not formal.** ~~Described G = (S, D, A, E) in ASCII and prose, but did not produce a formally-checkable adjacency list.~~ **Resolved:** `USAGE_PROCESS_GRAPH.dot` committed (47 nodes, 61 edges, 5-check verifier) + `platform/scripts/verify_graph_topology.py` pure-stdlib verifier. Current verification result: **PASS (5/5 checks)** — reachability, dead-end, acyclicity modulo bounded, decision determinism, failure recovery. Any future USAGE_PROCESS structural change must keep verifier green.
...
3. **[CLOSED 2026-04-24] Phase 10 → Phase 1 feedback loop is not mechanically bounded.** ~~No mechanism limits propagation of newly-learned rules into currently-active Tasks.~~ **Resolved:** ADR-022 Memory-version pinning per Execution (capture rule/microskill/guideline/invariant version_ids at pending→IN_PROGRESS transition via B.5 extension; append-only memory tables; in-flight Executions bound by their pin). USAGE_PROCESS_GRAPH.dot edge `A_MEMORY_UPDATE -> START` now has mechanical backing: `kind="bounded_feedback"` with ADR-022 reference in label. Verifier Check 3 enforces ADR reference presence.
