...
- *Input:* any client material — documents, meeting notes, specifications, emails, data samples — before it enters the evaluation, the codebase, or any AI-accessible memory.
...
- *Activity:* master plan (business intent); execution plan alongside the codebase (technical decisions); risks, decisions, edge cases, and verification criteria surfaced.
...
2. **Master Plan** — business intent, scope, success criteria.
...
1. **Structural format for each disclosure.** Abstract "disclose the assumption" degrades to silence under pressure. The contract must define **concrete labeled checkpoints** — fixed field names with mandatory slots — so that a slot being empty is itself a visible signal. Labels are `DID / DID NOT / CONCLUSION` (evidence report), `ASSUMING / VERIFIED / ALTERNATIVES` (pre-implementation), `MODIFYING / IMPORTED BY / NOT MODIFYING` (scope declaration), `DONE / SKIPPED / FAILURE SCENARIOS` (completion report). Any adopting organization may rename the labels, but must keep the property that a missing slot is detectable.
...
Framework Steward at each quarterly adoption audit verifies that the contract file still satisfies §4.4 clauses 1–8. A contract that has drifted (structural format abandoned, CONFIRMED loosened, non-trivial definition removed, subagent rules missing, 7 disclosure behaviors not enumerated, cascade compression rule delegated out of the contract) is treated with the same severity as a skipped Stage 1 gate: the framework is not operating.
...
**PII scanning and control.** Memory and content are subject to:
...
|---|---|
| Business logic owner | Confirms Stage 1 Evidence interpretations; signs off on Stage 2 Business-Level DoD; arbiter on domain-level decisions |
| Operations lead | Defines Stage 4 business-outcome verification events; provides access to target environments for verification |
...
- **Master Plan (artifact #2)** — client is reader and contributor
...
##### Canonical Example Already in project Practice
...
The pattern existed in project *before* CGAID formalized it. This subsection canonizes what was already working so that other adopters can implement it deliberately rather than re-discovering it accidentally.
...
- **Owners:** Project Engineering. Framework ownership follows a **scale-tiered governance model** (see subsection below), with **Lead Steward role rotating quarterly** in all tiers.
...
- **Handoff Document** — the standardized artifact transferring business intent to the codebase execution layer.
...
For projects's settlement-data work, DORA's ICT third-party risk framework applies to AI vendors processing settlement data. Stage 0 Confidential+ routing must verify AI vendor DPA terms against DORA Art. 28 obligations in addition to GDPR.
...
**Employment and labor**: AI systems used in employment decisions are classified as high-risk under Art. 6 of the EU AI Act. For any CGAID engagement producing AI features that influence hiring, performance management, or worker treatment, the "high-risk" pathway applies and conformity assessment artifacts become mandatory.
