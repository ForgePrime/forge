...
1. Read `skills/deep-orchestration/SKILL.md` → resolve orchestrator
2. Align on scope with user (restate + scoping questions)
3. Select skills: deep-explore + (deep-risk | deep-architect | deep-feasibility)
4. Gather context: `pipeline status`, `decisions read --status OPEN`, `lessons read-all`
5. Execute analysis via deep-orchestration, persist to `forge_output/{project}/research/`
6. Record findings: exploration decisions (OPEN), risk decisions (OPEN), architecture decisions
7. Present Discovery Brief → user decides: `/decide` or `/plan`
