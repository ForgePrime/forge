...
1. `pipeline next {project}` → get task
2. `pipeline context {project} {task_id}` → load deps + guidelines + risks
3. Execute: read code, implement, record decisions if non-trivial
4. `changes auto {project} {task_id} --reasoning "..."` → record changes (optional, auto on complete)
5. Verify: deep-verify + guidelines compliance check
6. `gates check {project} --task {task_id}` → run gates
7. `pipeline complete {project} {task_id} --reasoning "..."` → mark DONE
